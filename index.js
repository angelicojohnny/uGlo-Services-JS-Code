...
const slides = Array.from(document.querySelectorAll(".slide"));
    ...
    slide.classList.remove("active");
        ...
        const translateY = Math.abs(diff) * 50;
        const translateX = diff * 100;
        const zIndex = diff === 0 ? 0 : (diff < 0 ? diff : -diff);
    
        slide.style.transform = `rotate(${rotation}deg) translateY(${translateY}px) translateX(${translateX}%)`;
